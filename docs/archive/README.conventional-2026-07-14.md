...
**동기.** 융합(교과 통합) 수업 설계와 교육과정 지식 그래프 연구를 지원하기 위해,
교과 경계를 넘는 성취기준 간 연결을 데이터로 제공합니다. 설계 배경·알고리즘 아이디어·
파싱 이슈에 대한 서술형 문서는 [`docs/NARRATIVE.md`](./docs/NARRATIVE.md) 참고.
...
2단계 파이프라인. 코드: [`algorithm/`](./algorithm).
...
재현·확장을 위한 상세 명세(불변식·파라미터·프로토콜)는 [`algorithm/README.md`](./algorithm/README.md).
...
- 전체 목록: [`KNOWN_ISSUES.md`](./KNOWN_ISSUES.md).

## 로드맵

커버리지 확장(특히 1~2학년군) · candidate 검토 승격 · 억지 연결 교정 · 모델 갱신 후
재생성 · 표준 포맷(JSON-LD/CSV/그래프DB) export. 상세: [`ROADMAP`](./README.md#로드맵--아직-안-된-것들) / 이슈.
